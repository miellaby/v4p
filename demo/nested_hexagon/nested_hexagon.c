...
PolygonP create_star(Coord size, Color color) {
  PolygonP star = v4pPolygonNew(absolute, color, 10);
  ...
  for (i = 0; i < 10; i++) {
    ...
    Coord radius = (i % 2 == 0) ? size : size * 0.4f;     // Alternate between outer and inner points
    Coord x      = center_x + ((radius * lwmCosa) >> 8);  // Scale by 256 and shift right
    Coord y      = center_y + ((radius * lwmSina) >> 8);
    v4pPolygonAddPoint(star, x, y);
  }
...
PolygonP create_hexagon(Coord size, Color color) {
    PolygonP hex = v4pPolygonNew(absolute, color, 5);
...
Boolean g4pOnInit() {
    ...
    hexagon = create_hexagon(HEXAGON_SIZE, blue);
    v4pPolygonSetAnchorToCenter(hexagon);
    ...
    for (int i = 0; i < NUM_STARS; i++) {
        ...
        // Create star
        stars[i] = create_star(STAR_SIZE, red + (i * 4)); // Different colors for each star
        v4pPolygonSetAnchorToCenter(stars[i]);
        ...
        v4pPolygonAddSub(hexagon, stars[i]);
        v4pPolygonTransform(stars[i], star_x, star_y, 0, 0, 256, 256);
    }
    
    // clone the hexagon
    hexagon = v4pAddClone(hexagon);
    return success;
}
...
Boolean g4pOnTick(Int32 deltaTime) {
    ...
    int rotation_angle = (elapsedTime / 16) % 360; // Smooth rotation based on time
