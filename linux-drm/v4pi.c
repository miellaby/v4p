...
static int check_dumb_buffer_support() {
    ...
    if (drmGetCap(drm_context.fd, DRM_CAP_DUMB_BUFFER, &capability) < 0 || !capability) {
        v4pDisplayError("DRM device does not support dumb buffers");
        return -1;
    }
...
static drmModeEncoder* find_encoder_for_connector(drmModeRes *resources, drmModeConnector *connector) {
...
static int create_dumb_buffer(uint32_t width, uint32_t height, uint32_t bpp, 
    ...
    if (drmIoctl(drm_context.fd, DRM_IOCTL_MODE_CREATE_DUMB, &create) < 0) {
        v4pDisplayError("Failed to create dumb buffer");
        return -1;
    }
...
static uint8_t* map_dumb_buffer(uint32_t handle, uint32_t size) {
    ...
    if (drmIoctl(drm_context.fd, DRM_IOCTL_MODE_MAP_DUMB, &map) < 0) {
        v4pDisplayError("Failed to map dumb buffer");
        return NULL;
    }
    
    memory = mmap(0, size, PROT_READ | PROT_WRITE, MAP_SHARED, drm_context.fd, map.offset);
    if (memory == MAP_FAILED) {
        v4pDisplayError("Failed to mmap dumb buffer");
        return NULL;
    }
...
static int set_drm_mode() {
    // For dumb buffers, we need to create a framebuffer object to use with SetCrtc
    // This is the minimal required framebuffer setup
    uint32_t fb_id;
    uint32_t handles[4] = {drm_context.fb_handle};
    uint32_t pitches[4] = {drm_context.fb_stride};
    uint32_t offsets[4] = {0};
    
    if (drmModeAddFB2(drm_context.fd, drm_context.fb_width, drm_context.fb_height, 
                     DRM_FORMAT_XRGB8888, handles, pitches, offsets, &fb_id, 0) < 0) {
        v4pDisplayError("Failed to create DRM framebuffer for mode setting");
        return -1;
    }
    
    // Set the mode
    if (drmModeSetCrtc(drm_context.fd, drm_context.crtc_id, fb_id, 
                      0, 0, &drm_context.connector->connector_id, 1, &drm_context.mode) < 0) {
        v4pDisplayError("Failed to set CRTC mode");
...
static int init_drm() {
    ...
    if (drm_context.fd < 0) {
        v4pDisplayError("Failed to open DRM device /dev/dri/card0");
        return -1;
    }
    ...
    if (!drm_context.resources) {
        v4pDisplayError("Failed to get DRM resources");
        close(drm_context.fd);
        return -1;
    }
    ...
    if (!drm_context.connector) {
        v4pDisplayError("No connected connector found");
    ...
    if (!drm_context.encoder) {
        v4pDisplayError("No encoder found for connector");
...
Boolean v4pDisplayInit(int quality, Boolean fullscreen) {
    (void)quality; (void)fullscreen; // Unused for now
...
V4pDisplayP v4pDisplaySetContext(V4pDisplayP ctx) {
...
Boolean v4pDisplaySlice(Coord y, Coord x0, Coord x1, Color c) {
    ...
    if (y >= 0 && y < v4pDisplayContext->height && x0 >= 0 && x1 < v4pDisplayContext->width) {
        uint32_t *pixel = (uint32_t*)(v4pDisplayContext->framebuffer + y * v4pDisplayContext->stride);
...
Boolean v4pDisplayCollide(ICollide i1, ICollide i2, Coord py, Coord x1, Coord x2, PolygonP p1, PolygonP p2) {
    // Stub implementation
    (void)i1; (void)i2; (void)py; (void)x1; (void)x2; (void)p1; (void)p2;
    return false;
}
