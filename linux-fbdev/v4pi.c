...
Boolean v4pDisplayInit(int quality, Boolean fullscreen) {
  ...
  if (console_fd == -1) {
    v4pDisplayError("Error opening /dev/console");
    return failure;
  }

  if (ioctl(console_fd, VT_ACTIVATE, vt_num) == -1) {
    v4pDisplayError("Error switching virtual terminal");
    close(console_fd);
    return failure;
  }

  if (ioctl(console_fd, VT_WAITACTIVE, vt_num) == -1) {
    v4pDisplayError("Error waiting for virtual terminal switch");
    close(console_fd);
    return failure;
  }
  ...
  if (ioctl(console_fd, KDSETMODE, KD_GRAPHICS) == -1) {
    v4pDisplayError("Error setting console to graphics mode");
    close(console_fd);
    return failure;
  }
  close(console_fd);
  ...
  if (fbfd == -1) {
    v4pDisplayError("v4pDisplayInit failed, cannot open framebuffer device\n");
    return failure;
  }

  // Get variable screen information
  if (ioctl(fbfd, FBIOGET_VSCREENINFO, &v4pDisplayDefaultContextS.vinfo)) {
    v4pDisplayError("v4pDisplayInit failed, cannot get variable screen info\n");
    close(fbfd);
    return failure;
  }
  ...
  if (ioctl(fbfd, FBIOGET_FSCREENINFO, &v4pDisplayDefaultContextS.finfo)) {
    v4pDisplayError("v4pDisplayInit failed, cannot get fixed screen info\n");
    close(fbfd);
    return failure;
  }
  ...
  if (v4pDisplayDefaultContextS.vinfo.bits_per_pixel != 8) {
    ...
    if (ioctl(fbfd, FBIOPUT_VSCREENINFO, &v4pDisplayDefaultContextS.vinfo)) {
      ...
      v4pDisplayError("Warning: Cannot set 8-bit color mode, using %d-bit\n",
  ...
  v4pDisplayDefaultContextS.screensize  = v4pDisplayDefaultContextS.height * v4pDisplayDefaultContextS.line_length;

  // Map the framebuffer to memory
  v4pDisplayDefaultContextS.surface     = mmap(0, v4pDisplayDefaultContextS.screensize,
                                               PROT_READ | PROT_WRITE, MAP_SHARED, fbfd, 0);
  if (v4pDisplayDefaultContextS.surface == MAP_FAILED) {
    v4pDisplayError("v4pDisplayInit failed, cannot map framebuffer\n");
    close(fbfd);
    return failure;
  }
  ...
  if (v4pDisplayDefaultContextS.vinfo.bits_per_pixel == 8) {
    if (ioctl(fbfd, FBIOPUTCMAP, &palette)) {
      v4pDisplayError("Warning: Cannot set palette\n");
    }
  }
...
void v4pDisplayQuit() {
  ...
  int console_fd = open("/dev/console", O_RDWR);
  if (console_fd == -1) {
    v4pDisplayError("Error reopening /dev/console");
  } else {
    // Set the console back to text mode
    if (ioctl(console_fd, KDSETMODE, KD_TEXT) == -1) {
      v4pDisplayError("Error setting console to text mode");
      close(console_fd);
    }
    close(console_fd);
  }
