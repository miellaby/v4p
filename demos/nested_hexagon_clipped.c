...
int g4p_onInit(int quality, Boolean fullscreen) {
    ...
    // Create a clipping rectangle centered on the screen
    clip_rect = v4p_addNew(V4P_ABSOLUTE, V4P_GREEN, 0);
    v4p_addCorners(clip_rect,
        v4p_displayWidth / 4 - 3, v4p_displayHeight / 4 - 3, 
        v4p_displayWidth * 3 / 4 + 3, v4p_displayHeight * 3 / 4 + 3);
    v4p_addSub(clip_rect,
        v4p_addCorners(
            v4p_new(V4P_ABSOLUTE, V4P_GRAY, 1),
            v4p_displayWidth / 4, v4p_displayHeight / 4, 
            v4p_displayWidth * 3 / 4, v4p_displayHeight * 3 / 4));
...
int g4p_onTick(Int32 deltaTime) {
    ...
    int zoom_factor = 128 + 64 + (255 - iabs(phase - 128));  // Creates triangle wave: 128->0->128

    // Add horizontal sine wave movement
    int sin_angle = (elapsedTime / 32) % 512;  // Slower movement for sine wave
    computeCosSin(sin_angle);
    V4pCoord x_pos = (v4p_displayWidth / 2) + ((v4p_displayWidth / 4) * lwmSina >> 8);
    ...
    // Clip the hexagon to the rectangle in place
    v4p_clipClone(hexagon, hexagon, v4p_displayWidth / 4, v4p_displayHeight / 4, 
              v4p_displayWidth * 3 / 4, v4p_displayHeight * 3 / 4);
